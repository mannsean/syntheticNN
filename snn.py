...
class SyntheticNearestNeighbors:
    ...
    def _find_anchors(self, X, missing_pair):
        ...
        # get observed rows and columns
        obs_rows = X[:, missing_col].nonzero()[0]
        obs_cols = X[missing_row, :].nonzero()[1]
        assert missing_row not in obs_rows
        assert missing_col not in obs_cols
    ...
    def fit_transform(self, X, missing_set, covariates=None):
        """
        complete missing entries in matrix
        """
        predictions = np.empty(len(missing_set))
        for i, missing_pair in tqdm(enumerate(missing_set)):
            val, feasible = self._predict(
                X, missing_pair=missing_pair, covariates=covariates
            )
            predictions[i] = val if feasible else np.nan
        return predictions
